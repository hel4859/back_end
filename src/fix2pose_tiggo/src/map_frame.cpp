...
namespace util{
    ...
    sensor_msgs::NavSatFix MapFrame::MapFrame2GPS(){
        double lat = 360.0 / M_PI * atan(exp( (y_+OFFSET_Y+ORIGIN_Y) /(SCALE * EARTH_RAD_EQ)))-90.0;
        double lon = 180.0 * (x_ + OFFSET_X + ORIGIN_X) / (SCALE * EARTH_RAD_EQ * M_PI);
...
//        y = SCALE * EARTH_RAD_EQ * log(tan((90.0 + gps.latitude) * (M_PI / 360.0))) - OFFSET_Y;
    ...
    double MapFrame::getX()
    {
        return x_;
    }

    double MapFrame::getY()
    {
        return y_;
    }

    void MapFrame::setX(const double &x)
    {
        x_ = x;
    }

    void MapFrame::setY(const double &y)
    {
        y_ = y;
    }

    void MapFrame::setXY(const double &x, const double &y)
    {
        x_ = x;
        y_ = y;
    }
